...
- **DI:** Hilt 2.59.2
- **KSP:** 2.3.6 (standalone versioning — no longer tied to Kotlin version format since KSP 2.x)
...
- **Compose BOM:** All `androidx.compose.*` libraries should use the BOM for version management — do **not** specify explicit versions for BOM-managed artifacts (e.g., `material3`, `ui-text-google-fonts`).
- **Non-BOM Compose libs:** `hilt-navigation-compose` (androidx.hilt group) and Navigation 3 are outside the BOM and require explicit versions.
